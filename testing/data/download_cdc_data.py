...
def download_cdc_fluview_data(output_path: str = "cdc_flu_data.csv") -> pd.DataFrame:
    ...
    try:
        ...
        df["DATE"] = pd.to_datetime(
            df["YEAR"].astype(str) + df["WEEK"].astype(str).str.zfill(2) + "1", format="%Y%W%w"
        )
    ...
    for season in range(10):
        ...
        for week in range(52):
            ...
            for region in regions:
                ...
                ili_rate = (
                    1.0 + 4.0 * seasonal * np.random.uniform(0.5, 2.0) + np.random.normal(0, 0.3)
                )
                ...
                records.append(
                    {
                        ...
                        "%WEIGHTED ILI": ili_rate,
                        "ILI": int(ili_rate * total / 100),
...
def generate_synthetic_outbreak_data(output_path: str = "synthetic_flu_data.csv") -> pd.DataFrame:
    ...
    for season in range(15):
        ...
        for week in range(40):
            ...
            records.append(
                {
                    "season": f"{2008 + season}-{2009 + season}",
                    "week": week,
                    "date": week_date.strftime("%Y-%m-%d"),
                    "cases": int(cases),
                    "season_id": season,
